...
def roll_dice(num_rolls, dice=six_sided):
    ...
    n = num_rolls
    while n > 0:
        cur = dice() #roll a point
        if cur == 1:
            sum_one += 1
        else:
            sum += cur
        n -= 1
    return 1 if sum_one > 0 else sum
    # END PROBLEM 1
...
def free_bacon(score):
    ...
    "*** YOUR CODE HERE ***"
    cur = 100 - score
    while cur > 0:#find current units digit in pi
        pi //= 10 
        cur -= 1
    # END PROBLEM 2
...
def swine_align(player_score, opponent_score):
    ...
    "*** YOUR CODE HERE ***"
    gcd = 1
    factor = player_score if player_score < opponent_score else opponent_score
    
    while factor > 0:#find gcd between two scores
        if player_score % factor == 0 and opponent_score % factor == 0:
            gcd = factor
            break
        factor -= 1

    return player_score > 0 and opponent_score > 0 and gcd >= 10 and opponent_score >= 10
    # END PROBLEM 4a
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1):
        ...
        if who == 0:
            ...
            return announce_highest(who, score0, gain if gain > running_high else running_high)
                
        else:
            gain = score1 - last_score
            if gain > running_high:
                print(gain ,"point(s)! The most yet for Player", who)
            return announce_highest(who, score1, gain if gain > running_high else running_high)
...
def max_scoring_num_rolls(dice=six_sided, trials_count=1000):
    ...
    #         num = i
    #         print("IF a", a(i,dice))
    #     #     print("DEBUG_CHANGE ",num, a(i,dice), pre_max)
    #         pre_max = a(i,dice)
    #     #     print("endif")       
    #     print("DEBUG",i, a(i,dice), pre_max)
    #     i += 1
    num = 1
    i = 1
    cur_avg = 0
    ...
    while i <= 10:
        # average_roll_dice = make_averaged(roll_dice,trials_count)
        #刷新骰子???
        cur_avg = average_roll_dice(i,dice)
        if cur_avg > max_avg:
            num = i
            # print("CHANGE", num)
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    # Check if rolling 0 dice triggers an extra turn
    if extra_turn(score + free_bacon(opponent_score), opponent_score):
        return 0

    # Check if rolling 0 dice would give at least CUTOFF points and does not give an extra turn
    if free_bacon(opponent_score) >= cutoff:
        return 0
